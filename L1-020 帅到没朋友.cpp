...
#include <iostream>
using namespace std;
bool id[100000] = {false};    //存放某个朋友是否出现
long long w_id[10000] = {0};  //存放带查询的朋友ID
int main() {
    int n;
    cin >> n;
    for (int i = 0; i < n; ++i) {
    ...
    for (int i = 0; i < m; ++i) {
        cin >> w_id[i];
        if (id[w_id[i]] == false) {
            if (first_flag == 1) cout << " ";
            printf("%05lld", w_id[i]);
            id[w_id[i]] = true;
            first_flag = 1;
        }
    }
    if (first_flag == 0) cout << "No one is handsome";
    return 0;
}
